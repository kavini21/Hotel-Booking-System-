...
const AllRooms =()=>{
    ...
    const handleFilterChange = (checked, value, type) =>{
        setSelectedFilters((prevFilters)=>{
            ...
            const current = Array.isArray(updatedFilters[type]) ? updatedFilters[type] : [];
            if(checked){
                updatedFilters[type] = [...current, value];
            }else{
                updatedFilters[type] = current.filter(item => item !== value);
            }
    ...
    const handleSortChange = (sortOption)=>{
      ...
      // filter and sort rooms based on the selected filters ans sort option

                    const filteredRooms = useMemo(() => {
                        const list = Array.isArray(rooms) && rooms.length ? rooms : roomsDummyData;
                        const destination = searchParams.get('destination') || '';

                        const sortedAndFiltered = list
                            .filter((room) => {
                                // matchesRoomType
                                const roomTypeMatch = selectedFilters.roomType.length === 0 || selectedFilters.roomType.includes(room.roomType);

                                // matchesPriceRange
                                const priceMatch = !Array.isArray(selectedFilters.priceRange) || selectedFilters.priceRange.length === 0 || selectedFilters.priceRange.some((range) => {
                                    const [min, max] = range.split(' to ').map((s) => Number(s.trim()));
                                    return room.pricePerNight >= min && room.pricePerNight <= max;
                                });

                                // filterDestination
                                const destinationMatch = !destination || String(room?.hotel?.city || '').toLowerCase().includes(destination.toLowerCase());

                                return roomTypeMatch && priceMatch && destinationMatch;
                            })
                            .sort((a, b) => {
