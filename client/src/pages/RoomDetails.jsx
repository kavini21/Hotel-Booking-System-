...
const RoomDetails = () =>{
    ...
    const checkAvailability = async ()=>{
        try {
            ...
            if(checkInDate >= checkOutDate){
                toast.error('Check-In Date should be less thsn Check-Out Date')
                return;
            }
    ...
    // onSubmithandler funtion to check availability & book the room
                    ...
                    <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                        {room?.images.length > 1 && room.images.map ((image, index)=>(
                            <img onClick={()=> setMainImage(image)}
                             key={index} src={image} alt="Room Image" className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image ? 'ring-2 ring-orange-500' : ''}`}/>
                        ))}
                    </div>
                </div>
                        {/* Room Highights */}
                       ...
                       <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl'>
                            ...
                            {isAvailable ? "Book New" : "Check Availability" }  </button>
